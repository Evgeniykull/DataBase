...
void ModelUser::editUsers(int user_id) {
    ...
    add_user->show();
    connect(add_user, SIGNAL(onOkClick(int,int,QString,QString, int, QString)), SLOT(finishEditUser(int,int,QString,QString, int, QString)));
}

void ModelUser::finishAddUser(int parentId, QString name, QString shortName, int perm, QString type) {
    QSqlQuery* query = new QSqlQuery(data_base);
    QString statament = QString("INSERT INTO users (parentId, shortname, viewname, cardid, flags, ustype) VALUES (%1, %2, %3, %4, %5, %6)")
...
void ModelUser::finishEditUser(int userId, int parentId, QString name, QString shortName, int perm, QString type) {
    //date and dt compate
    //int dateUpdated = dateUpd ? 0 : 1;

    QSqlQuery* query = new QSqlQuery(data_base);
    QString statament = QString("UPDATE users SET parentid=%1, shortname=%2, viewname=%3, flags=%4, ustype=%5 WHERE userid=%6")
            .arg(QString::number(parentId))
            .arg("'" +  shortName + "'")
            .arg("'" + name + "'")
            .arg(QString::number(perm))
